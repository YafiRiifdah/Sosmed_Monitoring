...
export function SocialApiKeyPlaceholder({ description, icon, platformName }: Props) {
  return (
    <div className="space-y-6 text-[var(--text-muted)]">
      <section className="rounded-2xl border border-[var(--border-soft)] bg-[linear-gradient(135deg,var(--surface),var(--surface-muted))] p-5 shadow-sm backdrop-blur-xl">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-[var(--border-soft)] bg-[var(--surface)] text-[var(--accent)]">
            {icon}
          </div>
        </div>
      </section>
      ...
      <Card className="border-dashed border-[var(--border)] bg-[var(--surface-muted)] py-12">
        <EmptyState message={`Data API quota ${platformName} belum tersedia karena endpoint backend/API belum dibuat.`} />
      </Card>
    </div>
  );
}
