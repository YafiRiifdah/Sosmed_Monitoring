...
export function FacebookAccountManager({ kind }: { kind: AccountKind }) {
  const accountLabel = kind === "target" ? "akun target" : "akun wajib PAC";
  const title = kind === "target" ? "Facebook Target Accounts" : "Facebook Monitored Accounts";
  ...
  return (
    <div className="space-y-6 text-[var(--text-muted)]">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-subtle)]">
              Halaman {accountLabel} Facebook sudah disiapkan tanpa koneksi backend.
            </p>
          </div>

          <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-[var(--border-soft)] bg-[var(--surface)] text-[var(--accent-secondary)]">
            <Facebook size={22} />
          </div>
        </div>
      </section>
      ...
      <Card className="border-dashed border-[var(--border)] bg-[var(--surface-muted)] py-12">
        <EmptyState
          message={`Data ${accountLabel} Facebook belum tersedia karena endpoint backend/API belum dibuat.`}
        />
      </Card>
    </div>
  );
