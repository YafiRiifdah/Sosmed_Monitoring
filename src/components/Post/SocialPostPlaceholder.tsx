...
const platformMeta: Record<
  Exclude<PostPlatform, "instagram">,
  {
    label: string;
    icon: JSX.Element;
    tone: string;
...
export function SocialPostPlaceholder({
  ...
  platform: Exclude<PostPlatform, "instagram">;
}) {
  ...
  return (
    <div className="space-y-6 text-[var(--text-muted)]">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--text-subtle)]">
              Halaman post {meta.label} sudah disiapkan tanpa koneksi backend.
            </p>
          </div>
      ...
      <Card className="border-dashed border-[var(--border)] bg-[var(--surface-muted)] py-12">
        <EmptyState
          message={`Data post ${meta.label} belum tersedia karena endpoint backend/API belum dibuat.`}
        />
      </Card>
    </div>
  );
